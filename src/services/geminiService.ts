...
export const generateReportSummary = async (results: TestResult[]): Promise<ReportSummary> => {
  ...
  const prompt = `
    You are a PC Health Doctor expert system running on a diagnostic kiosk in a computer repair shop.
  ...
  try {
    ...
    console.warn("Online analysis service failed, falling back to offline report generator:", error);
